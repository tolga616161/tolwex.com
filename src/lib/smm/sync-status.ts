...
export async function syncOpenOrderStatuses(limit = 80) {
  const open = await prisma.smmOrder.findMany({
    where: {
      providerOrderId: { not: null },
      status: { in: ["pending", "processing", "inprogress", "in progress", "awaiting"] },
    },
  ...
  const ids = withIds.map((o) => o.providerOrderId!) ;
  ...
  for (const order of withIds) {
    ...
    await prisma.smmOrder.update({
      ...
      data: {
        status,
        startCounter: Number.isFinite(startCounter as number) ? (startCounter as number) : null,
        remains: Number.isFinite(remains as number) ? (remains as number) : null,
      },
    });
    updated += 1;

    // Partial / canceled refund remaining
    if (
    ...
    } else if (status === "partial" && remains != null && order.quantity > 0 && prev !== "partial") {
      const delivered = Math.max(0, order.quantity - Number(remains));
      const unused = order.quantity - delivered;
      if (unused > 0) {
        const refund = Math.round(((order.charge * unused) / order.quantity) * 10000) / 10000;
        if (refund > 0) {
          await adjustBalance(
            order.memberId,
            refund,
  ...
  return { checked: withIds.length, updated };
}
